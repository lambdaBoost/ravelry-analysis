#Script to fit a neural net to the "yarn_data" dataset through h2o.
#To run this script please ensure the following:
##1. The "data" image is saved in the working directory.
##2. The relevant lines are uncommented between rows 25 and 33 to determine whether the entire dataset or just a sample is used.
##3. Port 50001 is active (may use port 54321 if not)
##4. decide whether to standardise the data or not by uncommentin line 46 or 47 (doesn't seem to make much difference in practice)
#This script was run on EC2 and required 4 hours (the runtime is limited to 4 hours on line 95.
...
search_criteria=list(strategy="RandomDiscrete",max_runtime_secs = 14400,seed=57)
...
#view best model parameters and data
...
#plot actual and predicted values for best model
...
#get important variables and plot
